...
const botaoMostrarFavoritos = document.getElementById("botaoMostrarFavoritos"); // Novo botão
...
function criarElementoPersonagem(personagem) {
  ...
  divPersonagem.addEventListener("click", () => {
    alert(
      `Detalhes do personagem: ${personagem.name}\nStatus: ${personagem.status}\nEspécie: ${personagem.species}`
    );
  });
...
function exibirFavoritos() {
  ...
  if (favoritos.length === 0) {
    listaFavoritos.innerHTML = "<p>Nenhum favorito encontrado.</p>"; // Mensagem se não houver favoritos
  } else {
...
botaoNovosPersonagens.addEventListener("click", () => {
  const personagensAleatorios = todosPersonagens
...
// Adiciona evento ao botão para mostrar favoritos
botaoMostrarFavoritos.addEventListener("click", () => {
  exibirFavoritos(); // Chama a função para exibir os favoritos
});
